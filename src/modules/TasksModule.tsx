...
export function TasksModule() {
  ...
  const [showQueueListModal, setShowQueueListModal] = useState(false);
              ...
              <AnimatePresence>
                {tasks.map(task => (
                  ...
                  >
                    ...
                    <button onClick={(e) => { e.stopPropagation(); taskActions.remove(task.id!); }} className="opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity w-6 h-6 rounded-md hover:bg-red-500/20 flex items-center justify-center text-on-surface-variant hover:text-red-400 shrink-0 mt-0.5"><X className="w-4 h-4" /></button>
                  </motion.li>
                ))}
              </AnimatePresence>
            </ul>
      ...
      <div className="xl:col-span-4 flex flex-col gap-8">
        <div className="glass-panel rounded-2xl p-6 flex-1">
          ...
          <div className="flex flex-col gap-4">
             <button onClick={() => setShowQueueListModal(true)} className="p-4 rounded-xl bg-primary text-on-primary font-bold uppercase tracking-widest hover:bg-primary/90 transition-colors w-full flex items-center justify-center shadow-lg">
                View Session Queue
             </button>
      ...
      <AnimatePresence>
        {showQueueListModal && (
          <motion.div 
            initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
            onClick={() => setShowQueueListModal(false)}
          >
            <div className="bg-surface-container-high border border-white/10 p-6 rounded-2xl max-w-lg w-full max-h-[80vh] flex flex-col shadow-2xl relative" onClick={e => e.stopPropagation()}>
               <button onClick={() => setShowQueueListModal(false)} className="absolute top-4 right-4 text-on-surface-variant hover:text-white"><X className="w-5 h-5" /></button>
               <h4 className="text-xl font-bold mb-6">Session Queue</h4>
               
               <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-4">
                 {queue.length === 0 && <p className="text-on-surface-variant text-sm font-medium">Queue is empty.</p>}
                 {queue.map(q => {
                   ...
                   return (
                     <motion.div 
                        ...
                        onClick={() => {
                          if (!state.isFinished) {
                            loadTimer(q.id!, q.totalMins ? q.totalMins * 60 : 30 * 60, state.phase as 'focus' | 'break');
                            setShowQueueListModal(false);
                          }
                        }}
                        className={`p-4 rounded-xl flex flex-col gap-4 border transition-colors cursor-pointer group shadow-sm ${q.id === activeQueueId ? 'bg-primary/10 border-primary/40 ring-1 ring-primary/20' : 'bg-surface-container-highest/50 border-white/5 hover:border-white/20'} ${state.isFinished ? 'opacity-50 grayscale' : ''}`}
                     >
                       <div className="flex items-start gap-3 w-full">
                         ...
                         <div className="flex flex-col items-end gap-2 shrink-0">
                           <button
                             onClick={(e) => { e.stopPropagation(); queueActions.remove(q.id!); }}
                             className="opacity-100 transition-opacity w-8 h-8 rounded-md hover:bg-red-500/20 flex items-center justify-center text-on-surface-variant hover:text-red-400 shrink-0"
                           >
                             <X className="w-5 h-5" />
                           </button>
